                    ...
                    <div className="space-y-1.5">
                        {rankings.map((r, i) => (
                            <div key={r.id || i} className="flex items-center gap-2 p-1.5 rounded-lg bg-slate-50/50 dark:bg-black/20 hover:bg-white/50 dark:hover:bg-white/5 transition-colors group">
                                <div className={`w-5 text-center text-xs font-black ${i < 3 ? 'text-yellow-600 dark:text-yellow-400 drop-shadow-sm' : 'text-slate-500'}`}>
                                    {i + 1}
                                </div>
                                ...
                                <span className="text-xs font-bold text-slate-700 dark:text-slate-300 truncate flex-1 group-hover:text-blue-600 dark:group-hover:text-white transition-colors">{r.nickname}</span>
                                    ...
                                    {category === 'omega_unlimited' 
                                        ...
                                        : category === 'omega_quiz'
                                            ? <div className="flex flex-col items-end leading-none">
                                                <span className="text-slate-800 dark:text-white text-[10px]">{r.score} Streak</span>
                                                <span className="text-[8px] text-slate-500 dark:text-slate-400">Avg {r.subScore ? r.subScore : '0.00'}s</span>
                                              </div>
